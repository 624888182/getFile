...
public class N_PassInfo
{
    ...
    public string WebPassPOInfo(string connType, string connStr, string SapFlag, string TempPOID,string DBName)
    {
        ...
        string strhd = " select Top 1 podt.POID POID,so.NumberDefaultIndicator Sold_Code,so.Address_UF2 AG_NAME," +
                         " so.GivenName AG_STREET1,so.StreetName AG_STREET2,so.CareOfName AG_STREET3," +
                         " so.PostalCode AG_PostalCode,so.CityName AG_CITY,so.Address_UF1 AG_Address_UF1,so.CountryCode AG_LANDX,"+
                         " so.Address_UF3 AG_Address_UF3,so.Address_UF4 AG_Address_UF4,so.Address_UF5 AG_Address_UF5,so.Address_UF6 AG_Address_UF6," +
                         " sp.NumberDefaultIndicator Ship_Code,sp.CountryCode WE_LAND1," +
                         " sp.Address_UF2 WE_NAME,sp.GivenName WE_STREET1,sp.StreetName WE_STREET2,sp.CareOfName WE_STREET3," +
                         " sp.PostalCode WE_PostalCode,sp.CityName WE_CITY,sp.Address_UF1 WE_Address_UF1,sp.CountryCode WE_LANDX,"+
                         " sp.Address_UF3 WE_Address_UF3,sp.Address_UF4 WE_Address_UF4,sp.Address_UF5 WE_Address_UF5,sp.Address_UF6 WE_Address_UF6," +
                         " podt.ShipmentMode SHIP_Mode,podt.OriginalSOID TNS_SO,podt.FWCODE HMD_LSP," +
                         ...
                         " (select ContextText FROM " + DBName + ".[dbo].[POText] where POID=podt.POID and TypeCode='10011' and ITEMID='H') UF1,podt.OriginID UF2" +
                         ...
                         " where so.POID=sp.POID and so.ItemID=sp.ItemID and so.POID=pomt.POID and so.POID=podt.POID and so.ItemID=podt.ItemID and sp.POID=pomt.POID " +
                         " and sp.POID=podt.POID and sp.ItemID=podt.ItemID and pomt.POID=podt.POID and podt.POID='" + TempPOID + "' order by podt.ItemID";//so.Address_UF1
        ...
        //                 " substring(podt.PO_Create_DT_UF1,1,10) Delivery_Date,substring(podt.DeliveryStartDT,1,10) Ex_factory_date,podt.Orincoterms INCO1,podt.Orincoterms2 INCO2," +
        //                 " (select ContextText FROM " + DBName + ".[dbo].[POText] where POID=podt.POID and TypeCode='10014' and ITEMID='H') UF1,podt.OriginID UF2" +
        //                 " FROM " + DBName + ".[dbo].[POSoldToAddress] so," + DBName + ".[dbo].[POShipToAddress] sp," +
        //                 " " + DBName + ".[dbo].[PO_CREATE_MT] pomt," + DBName + ".[dbo].[PO_CREATE_DT] podt" +
        //                 " where so.POID=sp.POID and so.ItemID='H' and so.POID=pomt.POID and so.POID=podt.POID and sp.POID=pomt.POID " +
        //                 " and sp.POID=podt.POID and sp.ItemID=podt.ItemID and pomt.POID=podt.POID and podt.POID='" + TempPOID + "' order by podt.ItemID";//so.Address_UF1
        #endregion
        ...
        if (dthd.Rows.Count > 0)
        {
            for (int i = 0; i < dthd.Rows.Count; i++)
            {
                ...
                string AG_STREET = dthd.Rows[i]["AG_STREET3"] + " " + dthd.Rows[i]["AG_STREET2"] + " " + dthd.Rows[i]["AG_STREET1"];
                string AG_CITY = dthd.Rows[i]["AG_PostalCode"].ToString().Trim() + " " + dthd.Rows[i]["AG_CITY"].ToString().Trim();
                if(dthd.Rows[i]["AG_Address_UF1"].ToString().Trim()!="")
                {
                    AG_CITY += ',' + dthd.Rows[i]["AG_Address_UF1"].ToString().Trim();
                }
                string AG_LANDX = dthd.Rows[i]["AG_LANDX"].ToString().Trim();
                string AG_address_uf3 = dthd.Rows[i]["AG_Address_UF3"].ToString().Trim();
                string AG_address_uf4 = dthd.Rows[i]["AG_Address_UF4"].ToString().Trim();
                string AG_address_uf5 = dthd.Rows[i]["AG_Address_UF5"].ToString().Trim();
                string AG_address_uf6 = dthd.Rows[i]["AG_Address_UF6"].ToString().Trim();
                ...
                string WE_STREET = dthd.Rows[i]["WE_STREET3"] + " " + dthd.Rows[i]["WE_STREET2"] + " " + dthd.Rows[i]["WE_STREET1"];
                string WE_CITY = dthd.Rows[i]["WE_PostalCode"].ToString().Trim() + " " + dthd.Rows[i]["WE_CITY"].ToString().Trim() ;
                if(dthd.Rows[i]["WE_Address_UF1"].ToString().Trim()!="")
                {
                    AG_CITY += ',' + dthd.Rows[i]["WE_Address_UF1"].ToString().Trim();
                }
                string WE_LANDX = dthd.Rows[i]["WE_LANDX"].ToString().Trim();
                string WE_Address_UF3 = dthd.Rows[i]["WE_Address_UF3"].ToString().Trim();
                string WE_Address_UF4 = dthd.Rows[i]["WE_Address_UF4"].ToString().Trim();
                string WE_Address_UF5 = dthd.Rows[i]["WE_Address_UF5"].ToString().Trim();
                string WE_Address_UF6 = dthd.Rows[i]["WE_Address_UF6"].ToString().Trim();
                ...
                string UF1 = dthd.Rows[i]["UF1"].ToString().Trim();
                UF2 = dthd.Rows[i]["UF2"].ToString().Trim();
                dthead.Insert();
                ...
                dthead.CurrentRow.SetValue("AG_UF3", AG_address_uf3);
                dthead.CurrentRow.SetValue("AG_UF4", AG_address_uf4);
                dthead.CurrentRow.SetValue("AG_UF5", AG_address_uf5);
                dthead.CurrentRow.SetValue("AG_UF6", AG_address_uf6);
                ...
                dthead.CurrentRow.SetValue("WE_UF3", WE_Address_UF3);
                dthead.CurrentRow.SetValue("WE_UF4", WE_Address_UF4);
                dthead.CurrentRow.SetValue("WE_UF5", WE_Address_UF5);
                dthead.CurrentRow.SetValue("WE_UF6", WE_Address_UF6);
                ...
                dthead.CurrentRow.SetValue("UF1", UF1);
                dthead.CurrentRow.SetValue("UF2", UF2);
                dthead.CurrentRow.SetValue("ERDAT",System.DateTime.Now.ToString("yyyy-MM-dd"));
            }
            ...
            if (dtdt.Rows.Count > 0)
            {
                for (int ii = 0; ii < dtdt.Rows.Count; ii++)
                {
                    ...
                    dtitem.CurrentRow.SetValue("SKU_code", KDMAT);
                    ...
                    dtitem.CurrentRow.SetValue("UF2", UF2);
                }
            }
        }
        DataTable dtt1 = GetDataTableFromRFCTable(dthead);
        DataTable dtt2 = GetDataTableFromRFCTable(dtitem);
