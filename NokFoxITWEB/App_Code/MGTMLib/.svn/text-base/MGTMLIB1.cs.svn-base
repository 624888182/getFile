...
using Excel4  = Microsoft.Office.Interop.Excel ;
using System.Data;
using System.Text;
...
namespace DB.EAI
{
    ...
    public class MGTMLIB1
    {
        ...
        //    return bRet;
        //}
        public bool InsertDetail(string strUPDfile, string strDNnumber, string Customer, string sapModule, string country, string qty, string strshipdate, string sfcmodel, string strProgramx)
        {
            bool bRet = false;
            string strSql = string.Empty;
            string strSqlstrat = string.Empty;
            string strSqlrep = string.Empty;
            string Short_Str = strProgramx.Substring(0, 3).ToUpper();
            switch (Short_Str)
            {
                case "IME":
                    strSql = @"update SFC.UPD_DATALOAD_DETAIL_T set IMEI_FILE_TIME=sysdate  where INVOICE='" + strDNnumber + "'";

                    strSqlstrat = "INSERT INTO SFC.FTRAT1(TYPE,INVOICE,SHIP_QTY,Model,Ship_to,Customer,SHIP_DATE,Filename,CREATE_DATE,ACTION,SFC_MODEL,SAP_MODEL) " +
                         "VALUES('IMEI','" + strDNnumber + "','" + qty + "','" + sapModule + "','" + country + "','" + Customer + "'," +
                         "TO_DATE('" + Convert.ToDateTime(strshipdate).ToString("yyyy/MM/dd") + "','YYYY/MM/DD')" + ",'" + strUPDfile + "',sysdate,'NORMAL','" + sapModule + "','" + sfcmodel + "')";
                    break;
                case "UPD":
                    strSql = "INSERT INTO SFC.UPD_DATALOAD_DETAIL_T(INVOICE,SHIP_QTY,Model,Ship_to,Customer,SHIP_DATE,Filename,CREATE_DATE,ACTION,SFC_MODEL,SAP_MODEL) " +
                         "VALUES('" + strDNnumber + "','" + qty + "','" + sapModule + "','" + country + "','" + Customer + "'," +
                         "TO_DATE('" + Convert.ToDateTime(strshipdate).ToString("yyyy/MM/dd") + "','YYYY/MM/DD')" + ",'" + strUPDfile + "',sysdate,'NORMAL','" + sapModule + "','" + sfcmodel + "')";

                    strSqlstrat = "INSERT INTO SFC.FTRAT1(TYPE,INVOICE,SHIP_QTY,Model,Ship_to,Customer,SHIP_DATE,Filename,CREATE_DATE,ACTION,SFC_MODEL,SAP_MODEL) " +
                         "VALUES('UPD','" + strDNnumber + "','" + qty + "','" + sapModule + "','" + country + "','" + Customer + "'," +
                         "TO_DATE('" + Convert.ToDateTime(strshipdate).ToString("yyyy/MM/dd") + "','YYYY/MM/DD')" + ",'" + strUPDfile + "',sysdate,'NORMAL','" + sapModule + "','" + sfcmodel + "')";

                    break;
                case "ASN":
                    strSql = @"update sfc.UPD_DATALOAD_DETAIL_T set ASN_FTP_TIME=sysdate  where INVOICE='" + strDNnumber + "'";

                    strSqlstrat = "INSERT INTO SFC.FTRAT1(TYPE,INVOICE,SHIP_QTY,Model,Ship_to,Customer,SHIP_DATE,Filename,CREATE_DATE,ACTION,SFC_MODEL,SAP_MODEL) " +
                         "VALUES('ASN','" + strDNnumber + "','" + qty + "','" + sapModule + "','" + country + "','" + Customer + "'," +
                         "TO_DATE('" + Convert.ToDateTime(strshipdate).ToString("yyyy/MM/dd") + "','YYYY/MM/DD')" + ",'" + strUPDfile + "',sysdate,'NORMAL','" + sapModule + "','" + sfcmodel + "')";
                    break;
                case "EDI":
                    //Result_Str = Get_EDI(strInvoice, strProgram);
                    break;
                case "GFS":
                    strSql = @"update sfc.UPD_DATALOAD_DETAIL_T set GFS_FILE_TIME=sysdate  where INVOICE='" + strDNnumber + "'";

                    strSqlstrat = "INSERT INTO SFC.FTRAT1(TYPE,INVOICE,SHIP_QTY,Model,Ship_to,Customer,SHIP_DATE,Filename,CREATE_DATE,ACTION,SFC_MODEL,SAP_MODEL) " +
                         "VALUES('GFS','" + strDNnumber + "','" + qty + "','" + sapModule + "','" + country + "','" + Customer + "'," +
                         "TO_DATE('" + Convert.ToDateTime(strshipdate).ToString("yyyy/MM/dd") + "','YYYY/MM/DD')" + ",'" + strUPDfile + "',sysdate,'NORMAL','" + sapModule + "','" + sfcmodel + "')";
                    break;
                default:
                    break;
            ...
            try
            {
                using (OracleDB oraDB = new OracleDB())
                {
                    bRet = oraDB.ExecuteNonQuery(strSqlstrat);
                }
            }
            catch (Exception ex)
            {
        ...
        {
            foreach (System.Diagnostics.Process pro in System.Diagnostics.Process.GetProcessesByName("Excel4."))
            {
        ...
        public string GetAllInvoiceNumberHanle(string txtInvoice)
        {
            string strRet = string.Empty;
            using (OracleDB oraDB = new OracleDB())
            {
                string strInvoice = string.Empty;
                string strSql = string.Empty;
                strSql = @"select DISTINCT A.PLANT, A.invoice, A.customer_name,A.ITEM_MODULE,UPPER(b.Ship_To_Country),
                                      (select sum(quantity) from CMCS_SFC_PACKING_LINES_ALL b where b.INVOICE_NUMBER=a.invoice) shipped_qty,
                                      LAST_SHIPMENT_DATE ,C.CUSTOMER_TYPE from SFC.MES_MIT_INVOICE A,CMCS_SFC_PACKING_LINES_ALL B ,SFC.CMCS_SFC_MODEL C
                                      Where A.Invoice = B.INVOICE_NUMBER
                                      ...
                                      or UPPER(A.CUSTOMER_NAME) LIKE '%SUTECH%' or UPPER(A.CUSTOMER_NAME) LIKE '%TX4-SOI%' or 
                                      A.CUSTOMER_NAME LIKE 'FIH (Hong Kong) Limited'
                                      or UPPER(A.CUSTOMER_NAME) LIKE '%MI-AMK%' or UPPER(A.CUSTOMER_NAME) LIKE '%S&B INDUSTRY INC.%') and  INVOICE='" + txtInvoice.Trim() + "'  ";
        ...
        #region Get UPD,IMEI,ASN,GFS(Alter-8/19-8/21)

        public bool Get_AllData(string strInvoice /*發票號*/, string strProgram  /*程式編碼*/,int status/*1-Hand,2-Auto*/)
        {
            ...
            if (status == 2)
            {
                int iRet = sapo.CheckDNMovementStatus(strInvoice);
                switch (iRet)
                {
                    case 0:
                        break;
                ...
                string Short_Str = strProgram.Substring(0, 3).ToUpper();
                switch (Short_Str)
                {
                    case "IME":
                        Result_Ret = Get_IMEI(strInvoice, strProgram);
                        break;
                    case "UPD":
                        Result_Ret = Get_UPD(strInvoice, strProgram);
                        break;
                    case "ASN":
                        Result_Ret = Get_ASN(strInvoice, strProgram);
                        break;
                    case "EDI":
                        Result_Ret = Get_EDI(strInvoice, strProgram);
                        break;
                    case "GFS":
                        Result_Ret = Get_GFS(strInvoice, strProgram);
                        break;
                    default:
                        break;
                }
        ...
        public bool Get_UPD(string strInvoice, string strProgram)
        {
            ...
            string strshipcountry = string.Empty;
            string strshipcity = string.Empty;
            ...
            string strFileName;
            bool bRet = false;
            string[,] Invoice = AutoCheck(strInvoice, "0", "UPD", "");

            if (Invoice.Length < 0)
            {
                AddMail("UPD FILE", "", "", "no shipment records", "");
                ErrorMessage = "No Have DN shipment records";
                Inserterror(strInvoiceNumber, ErrorMessage, "No Have DN shipment records");
            }
            else
            {
                ...
                {
                    strPlant = Invoice[i, 0].ToString();
                    strInvoiceNumber = Invoice[i, 1].ToString();
                    strcustomername = Invoice[i, 2].ToString();
                    strsapmodel = Invoice[i, 3].ToString();
                    strshipcountry = Invoice[i, 4].ToString();
                    strshipcity = Invoice[i, 5].ToString();
                    strqty = Invoice[i, 6].ToString();
                    strsfcmodel = Invoice[i, 7].ToString();
                    strshipdate = Invoice[i, 8].ToString();
                    strcustomertype = Invoice[i, 9].ToString();
                }
                string strCheckpo = checkPo(strInvoiceNumber);//  Check its have get Po!
                if (strCheckpo.Length == 0)
                {
                    ErrorMessage = "P.O. Not Found in System, Please make sure you have entered the P.O. Info into the system.";
                    Inserterror(strInvoiceNumber, ErrorMessage, "");
                }
                else
                {
                    List<StringBuilder> lsb = null;
                    lsb = GetUPDInfo(strPlant, strInvoiceNumber, strsapmodel, strshipcountry, strsfcmodel, strqty, strProgram);
                    if (lsb.Count == 0)
                    {
                        ErrorMessage = "No Qurey Data,Please Information IT";
                        Inserterror(strInvoiceNumber, ErrorMessage, "");
                        AddMail("UPD FILE Invoice : " + strInvoiceNumber, "", "", "No Qurey Data,Please Information IT", "");
                    }
                    else
                    {
                        for (int j = 0; j < lsb.Count; j++)
                        {
                            ...
                            if (CreateFile(lsb[j], strUPDFileName))
                            {
                                // File.Move(strIMEIFileName, IMEI_FILE_BAK + "\\" + strFileName);
                                File.Copy(strUPDFileName, UPD_FILE_BAK + "\\" + strFileName);
                                InsertDetail(strFileName, strInvoiceNumber, strcustomername, strsapmodel, strshipcountry, strqty, strshipdate, strsfcmodel, strProgram);
                                bRet = true;
                            }
        ...
        public bool Get_IMEI(string strInvoice, string strProgram)
        {
            ...
            string[,] Invoice = AutoCheck(strInvoice, "1", "UPD", "");
            ...
            else
            {
                ...
                strIMEIFileName = CreateIMEIFile(strInvoiceNumber, strsapmodel, strsfcmodel, strshipcountry, strProgram);
                // UpdateImeiTime(strInvoiceNumber, sendcount);
                if (strIMEIFileName.Length == 0)
                {
                    ErrorMessage = "Create IMEI File error,Please Information IT";
                    Inserterror(strInvoiceNumber, ErrorMessage, "");
                    AddMail("UPD FILE Invoice : " + strInvoiceNumber, "", "", "Create IMEI File error,Please Information IT", "");
                }
                else
                {

                    string[] strTmp = strIMEIFileName.Split('\\');
                    strFileName = strTmp[strTmp.Length - 1];
                    File.Move(strIMEIFileName, IMEI_FILE_BAK + "\\" + strFileName);
                    InsertDetail(strFileName, strInvoiceNumber, strcustomername, strsapmodel, strshipcountry, strqty, strshipdate, strsfcmodel, strProgram);
                    bret = true;
        ...
        public bool Get_ASN(string strInvoice, string strProgram)
        {
            string strPlant = string.Empty;
            string strInvoiceNumber = string.Empty;
            string strcustomername = string.Empty;
            ...
            else
            {
                ...
                strSqlqty = @"SELECT ship_qty,filename  FROM SFC.UPD_DATALOAD_DETAIL_T   WHERE  invoice= '" + strInvoice + "'";// CREATE_TIME>SYSDATE-1 AND
                using (OracleDB oraDB = new OracleDB())
                {
                    try
                    {
                ...
                if (strshipqty.Length > 0)
                {
                    try
                    {
                        ...
                        if (ds != null)
                        {
                            ...
                            InsertDetail(strASNFileName, strInvoiceNumber, strcustomername, strsapmodel, strshipcountry, strqty, strshipdate, strsfcmodel, strProgram);
                            bret = true;
                        }
                        else
                        {
                            AddMail("ASN FILE", "", "", "DownLoad Invoice Info Error", "");
                            ErrorMessage = "ASN DownLoad Invoice Info Error";
                            Inserterror(strInvoiceNumber, ErrorMessage, "DownLoad Invoice Info Error");
                ...
                {
                    AddMail("ASN FILE", "", "", "UPD filename upload error!", "");
                    ErrorMessage = "UPD filename upload error!";
                    Inserterror(strInvoiceNumber, ErrorMessage, "UPD filename upload error!");
                    OracleDB.Write("UPD filename upload error! , Invoice : " + strInvoice);
                }

            }
            return bret;
        }　　//生成ASN　文檔
        public bool Get_GFS(string strInvoice, string strProgram)
        {
            string strPlant = string.Empty;
            string strInvoiceNumber = string.Empty;
            string strcustomername = string.Empty;
            ...
            else
            {
                ...
                List<StringBuilder> lsb = null;
                lsb = GetGFS(strInvoice);
                if (lsb.Count == 0)
                {
                    AddMail("GFS FILE", "", "", "invoice : " + strInvoice + "  no data", "");
                }
                else
                {
                    for (int k = 0; k < lsb.Count; k++)
                    {
                        ...
                        strGFSFileName = string.Format(GFS_FILE_NAME, dt.ToString("yyyyMMddHHmmss"), strInvoice.Replace("'", ""), strRand);
                        string strBKGFSFileName = GFS_FILE_BAK + GetFileName(strGFSFileName);
                        string[] strTmp = strGFSFileName.Split('\\');
                        strFileName = strTmp[strTmp.Length - 1];
                        // string strBKGFSFileName = Application.StartupPath + @"\GFS\" + GetFileName(strGFSFileName);
                        if (CreateGFSFile(lsb[k], strGFSFileName, dt.ToString("yyyyMMddHHmmss"), strInvoice.Replace("'", "") + "-" + strRand, "FIHTJ", "V01", strInvoice))
                        {
                            File.Copy(strGFSFileName, strBKGFSFileName);
                            bRet = true;
                            continue;
                        }
                        else
                        {
                            ErrorMessage = "Create GFS File error,Please Information IT";
                            Inserterror(strInvoiceNumber, ErrorMessage, "");
                            AddMail("UPD FILE Invoice : " + strInvoiceNumber, "", "", "Create GFS File error,Please Information IT", "");
                            File.Delete(strGFSFileName);
                            continue;
                        }
        ...
        public bool Get_EDI(string strInvoice, string strProgram)
        {
            bool bRet = false;
            return bRet;
        }     //生成EDI　文檔
        ...
        public List<StringBuilder> GetUPDInfo(string strPlantx, string strInvoice, string strsapmodelx, string strshipcountryx,
            string strsfcmodelx, string qtyx, string strProgram)
        {
            ...
            switch (strProgram)
            {
                // 機 種	編 號	UPD	    IMEI	ASN 	EDI	    GFS
                //===========================================================
                // FD1   	001 	UPD01	IMEI01	EDI01	EDI01	EDI01
                // QAZ   	002 	UPD02	IMEI02	EDI01	EDI01	EDI01
                // DMQ   	003 	UPD03	IMEI03	EDI01	EDI01	EDI01
                // QAZ-DS	004 	UPD04	IMEI04	EDI01	EDI01	EDI01
                // A3G	    005 	UPD05	IMEI05	EDI01	EDI01	EDI01
                // A2G   	005	    UPD05	IMEI05	EDI01	EDI01	EDI01
                case "IMEI01":
                case "UPD01":
                ...
                case "UPD02":
                    Sqlstr = @"SELECT DISTINCT 'IMEI' f1, c.imei f2,'" + FactoryCode + "' f3,"
                                       ...
                                       + "WHERE a.order_carton_no = b.internal_carton "
                                         ...
                                         + "AND ( c.imei = e.imei  "
                                              + "AND e.status = 'PASS' "
                                              + "AND e.e2pdate =(SELECT MAX (h.e2pdate) FROM " + strsapmodelx + ".e2pconfig h "
                    ...
                    Sqlstr = @"SELECT DISTINCT 'IMEI' f1, c.imei f2, 'ZFOXTJ-ODM' f3,"
                                       ...
                                       + "WHERE a.order_carton_no = b.internal_carton "
                                         ...
                                         + "AND (c.imei = e.imei "
                                              + "AND e.status = 'PASS' "
                                              + "AND e.e2pdate = "
                                                          + "(SELECT MAX (h.e2pdate) FROM " + strsapmodelx + ".e2pconfig h WHERE h.imei = c.imei "
                                              + "AND h.status = 'PASS')) "
                                           + "AND j.carton_no = b.internal_carton "
                                           + "AND UPPER (b.ship_to_country) = k.country_code "
                                           + "AND b.invoice_number = a.invoice_no "
                ...
                default:
                   ...
                   + "AND (c.imei = e.imei "
                         + "AND e.status = 'PASS'"
                         + "AND e.e2pdate =(SELECT MAX (h.e2pdate) FROM " + strsapmodelx + ".e2pconfig h WHERE h.imei = c.imei AND h.status = 'PASS')) "
                   + "AND j.carton_no = b.internal_carton "
                   + "AND UPPER (b.ship_to_country) = k.country_code "
                   + "AND a.invoice_no ='" + strInvoice + "'";
                    break;
        ...
        public void ExitProgram(string strMsg, string strKeyWord)
        {
            // Log.Write(strMsg);
            //Log.Write("[SUCCEED] end");
        }

        public string Get_tra_UPD(string strMsg, string strKeyWord)
        {
            // Log.Write(strMsg);
            //Log.Write("[SUCCEED] end");
            return "";
        }
        private string CreateIMEIFile(string strInvoice, string strsfcModel, string strsapmodel, string strcountry, string strProgram)
        {
            // 機 種	編 號	UPD	    IMEI	ASN 	EDI	    GFS
            //===========================================================
            // FD1   	001 	UPD01	IMEI01	ASN01	EDI01	GFS01
            // QAZ   	002 	UPD02	IMEI02	ASN02	EDI02	GFS02
            // DMQ   	003 	UPD03	IMEI03	ASN03	EDI03	GFS03
            // QAZ-DS	004 	UPD04	IMEI04	ASN04	EDI04	GFS04
            // A3G	    005 	UPD05	IMEI05	ASN05	EDI05	GFS05
            // A2G   	005	    UPD05	IMEI05	ASN05	EDI05	GFS05
            string strIMEIFile = "-";
            string strCustomerName = string.Empty;
            ...
            if (strProgram == "IMEI05" && strcountry == "LA")
            {
                strSql = @"SELECT   e.so_no sales_po, g.ppart fih_pn, g.hw_ver hw, e.so_no so,
                                 ...
                                 DECODE(c.imeinum2,'','N/A',c.imeinum2) imei2, SUBSTR (h.th7, 16, 8) p_uc,
                                 SUBSTR (h.th7, 48, 8) s_uc, 'N/A' s_pc
                            FROM cmcs_sfc_packing_lines_all a,
                                 shp.cmcs_sfc_shipping_data b,
                                 shp.cmcs_sfc_imeinum c,
                                 shp.cmcs_sfc_carton d,
            ...
            if (strProgram == "IMEI03" && strcountry == "LA")
            {
                strSql = @"SELECT   e.so_no sales_po, g.ppart fih_pn, g.hw_ver hw, e.so_no so,
                             g.sw_ver sw, LPAD (e.invoice, 10, '0') invoice_no,
                             g.phone_model xcvr_no,
                             ...
                             TO_CHAR (a.ship_date, 'yyyy/mm/ dd') ship_date, d.caseid carton_id,
                             LPAD (a.invoice_number, 10, '0') || '+1095759' pallet_id,
                             DECODE (c.imeinum2, '', 'N/A', c.imeinum2) imei2, DECODE(h.nkey,'','N/A',h.nkey) p_uc,
                             DECODE(h.nskey,'','N/A',h.nskey) s_uc, 'N/A' s_pc
                        FROM cmcs_sfc_packing_lines_all a,
                             shp.cmcs_sfc_shipping_data b,
                             shp.cmcs_sfc_imeinum c,
                             shp.cmcs_sfc_carton d,
                       ...
                       WHERE a.invoice_number =" + strInvoice + @"
                         ...
                         AND (    c.imeinum = h.imei
                              ...
                              AND h.e2pdate = (SELECT MAX (h.e2pdate)
                                                 FROM " + strsapmodel + @".e2pconfigtw h
            ...
            Excel4.Application app = new Excel4.Application();
            ...
            using (OracleDB oraDB = new OracleDB())
            {
                ...
                try
                {
                    Excel4.Workbook wb = app.Workbooks.Add(true);
                    Excel4.Worksheet st = (Excel4.Worksheet)app.Sheets[1];
                    ...
                    if (strsfcModel == "QAZ")
                    {
                        ...
                        if (strCustomerName.Equals("CONCORDE S.P.A."))
                        {
                            ...
                            Excel4.Range range = st.get_Range(st.Cells[n, 1], st.Cells[n + dt.Rows.Count - 1, 7]);
                        ...
                        else
                        {
                            ...
                            Excel4.Range range = st.get_Range(st.Cells[n, 1], st.Cells[n + dt.Rows.Count - 1, 8]);
                            range.NumberFormatLocal = "@";
                            range.Value2 = objData;
                    ...
                    else if ((strsfcModel == "A3G" || strsfcModel == "A2G") && strcountry == "LA")
                    {
                        foreach (DataRow dr in dt.Rows)
                        {
                            if (!bInit)
                        ...
                        else
                        {
                            ...
                            Excel4.Range range = st.get_Range(st.Cells[n, 1], st.Cells[n + dt.Rows.Count - 1, 10]);
                            range.NumberFormatLocal = "@";
                            range.Value2 = objData;
                    ...
                    else if (strsfcModel == "DMQ" && strcountry == "LA")
                    {
                        foreach (DataRow dr in dt.Rows)
                        {
                            if (!bInit)
                    ...
                    wb.SaveAs(strIMEIFile, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Excel4.XlSaveAsAccessMode.xlExclusive, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing); //Excel4.XlFileFormat.xlExcel8
                ...
                {
                    //  Log.Write("[ERROR] GetInvoiceInfo " + ex.Message);
                    app.Quit();
                    DateTime Process_AfterTime = DateTime.Now;
                    KillExcelProcess(Process_BeforeTime, Process_AfterTime);
                    GC.Collect();
        ...
        public string Get_tra_IMEI_NEW(string strInvoice, string strIMEICode, string strModel, string strcountry)
        {
            ...
            //  2. IF Have Data,so Create Data to Excel4.,Saved to the specified directory
            //  3. Return a Excel4. File Name.
            /////////////////////////////////////////////////
            string strIMEIFile = "-";
        ...
        public void Get_tra_ASN_NEW(string strInvoice, string strDate, string supdfile)
        {
            ...
            SAPRFC.SAPFunction sapf;
            sapf = new SAPRFC.SAPFunction();
            sapf = new SAPRFC.SAPFunction("10.134.28.98", "802", "RFCTJIDM", "TJSAPRFC", "00");  //zmf  正式802 環境
            string strSqlqty = string.Empty;
            string strshipqty = string.Empty;
            string strSql1 = string.Empty;
            strSqlqty = @"SELECT ship_qty  FROM SFC.UPD_DATALOAD_DETAIL_T   WHERE  invoice= '" + strInvoice + "'";// CREATE_TIME>SYSDATE-1 AND
        ...
        public List<StringBuilder> GetGFS(string strInvoice)
        {
            List<StringBuilder> lsb = new List<StringBuilder>();
            string strSql = string.Empty;
            strSql = @"SELECT 'UN' F1,'CM' F2,rownum F3,C.CUSTOMER_NUM F4,C.SERIAL_NUM F5,
        ...
        public string[,] AutoCheck(string p1, string p2, string type, string p4)
        {
            ...
            string sqlCondition5 = " AND a.last_shipment_date >sysdate-" + p4 + " ";
            ...
            if (p1.ToUpper() != "ALL")
            {
                if (p2 == "0")
                {
                    ...
                    strSql = strSql + sqlCondition1 + " AND a.invoice='" + p1 + "' ";
        ...
        public string[,] ShipMentInfoQuery(string p1, string p2, string type, string p4,string p5)
        {
            /// If p1=DN_NO :Means manual generate UPD file
            /// If p1=ALL   :Means auto chk data and generate UPD File
            string strRet = string.Empty;
            ...
            string sqlCondition3 = " AND a.invoice NOT IN (SELECT invoice FROM shp.upd_dataload_detail_t) ";
            string sqlCondition4 = " AND a.invoice  IN (SELECT invoice FROM shp.upd_dataload_detail_t) ";
            string sqlCondition5 = " AND to_char( a.last_shipment_date,'yyyy/MM/dd') between '" + p4 + "'and'" +p5+ "'";
            string SqlSort = " ORDER BY a.last_shipment_date DESC ";
            strSql = @"SELECT DISTINCT a.plant, a.invoice, a.customer_name, a.item_module,
                                b.ship_to_country, b.ship_to_city,
                                a.SHIPPED_QTY shipped_qty,d.model,
        ...
        void AsnFileFromSAP(string strInvoice, string strDate, string supdfile)
        {
            DataSet ds = GetInvoiceFromSAP(strInvoice);
            ...
            string strSqlqty = string.Empty;
            string strshipqty = string.Empty;
            strSqlqty = @"SELECT ship_qty  FROM SHP.UPD_DATALOAD_DETAIL_T   WHERE  invoice= '" + strInvoice + "'";// CREATE_TIME>SYSDATE-1 AND
            using (OracleDB oraDB = new OracleDB())
            {
                try
                {
                ...
                {
                    //  Log.Write("[ERROR] SHP.UPD_DATALOAD_DETAIL_T shipqty is error " + ex.Message);
                    return;
                }
            }

            //if (((DataTable)dgHead.DataSource).Rows.Count > 0)
            //{
            DataTable dtHead = (DataTable)ds.Tables[0];
            DataTable dtDetail = (DataTable)ds.Tables[1];


            StringBuilder sbs = new StringBuilder();
            sbs.Append(dtHead.Rows[0][0].ToString() + "|" + DateTime.Now.ToString("dd/MM/yyyy") + "|" + dtDetail.Rows[0]["BSTKD"].ToString() + "|");

            sbs.Append(Int16.Parse(dtDetail.Rows[0]["POSEX"].ToString()).ToString() + "|" + dtDetail.Rows[0]["KDMAT"].ToString() + "|" + strshipqty + "|");
            ...
            using (OracleDB oraDB = new OracleDB())
            {
                ...
                {
                    // Log.Write("[ERROR] Get FTP_FOLDER Error " + ex.Message);
                    return;
                }
            }
            string strSql1 = string.Empty;
            strSql1 = @"update SHP.UPD_DATALOAD_DETAIL_T   set  asn_ftp_time=sysdate where invoice='" + strInvoice + "'";
            bool strok;
        ...
        private DataSet GetInvoiceFromSAP(string strInvoice)
        {
            SAPRFC.SAPFunction sapf;
            if (true)
                sapf = new SAPRFC.SAPFunction();
            ...
            {
                //sapf = new SAPRFC.SAPFunction("10.134.140.157", "502", "TEST01", "FOXCONN7", "00");
                //sapf = new SAPRFC.SAPFunction("10.134.92.27", "802", "rfctymcm", "foxconn8", "00");  // 測試802
                sapf = new SAPRFC.SAPFunction("10.134.28.98", "802", "RFCTJIDM", "TJSAPRFC", "00");  //zmf  正式802 環境
            }
            //sapf = new SAPRFC.SAPFunction("10.134.28.76", "802", "sapsfc", "init");
            ...
            if (ds != null)
            {
                DataTable dtHead = ds.Tables[0];
                DataTable dtDetail = ds.Tables[1];
                //dgHead.DataSource = null;
                //dgDetail.DataSource = null;
                //dgHead.DataSource = dtHead;
        ...
        public string[,] Checkmaster(string motocustomer, string sapmodel, string sfcmodel, string shipto)
        {
            int iColNum, iRowNum;
            string[,] arraydata = new string[0, 0];
            string strSql = string.Empty;
            strSql = @"SELECT DISTINCT SHIP_TO,AUTOFLAG,F_TYPE,F_TYPE2,F_TYPENO,SAP_MODEL,SFC_MODEL FROM FTRAM1 WHERE MOTO_CUSTOMER='" + motocustomer + "' AND MODEL='" + sapmodel + "' AND SFC_MODEL='" + sfcmodel + "' AND SHIP_TO='" + shipto + "' ORDER BY F_TYPENO DESC";
