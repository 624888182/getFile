...
function writeIframe() {
    ...
    "    <td title='前一年' onclick='parent.prevY()' style='cursor: hand' class=bg><<</td>" +
    "    <td width=15 height=19 class=bg title='前一月' style='cursor: hand;color:red;' onclick='parent.prevM()'><</td>" +
    "    <td width=60 id=meizzYearHead  title='選擇年份' onclick='parent.funYearSelect(parseInt(this.innerText, 10))'" +
    "        onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'" +
    "        onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'></td>" +
    "    <td width=50 id=meizzYearMonth title='選擇月份' onclick='parent.funMonthSelect(parseInt(this.innerText, 10))'" +
    "        onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'" +
    "        onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'></td>" +
    "    <td width=15 class=bg title='下一月' onclick='parent.nextM()' style='cursor: hand;color:red;'>></td>" +
    "    <td title='下一年' onclick='parent.nextY()' style='cursor: hand' class=bg>>></td>" +
    "</tr></table>" +
    ...
    strIframe += "<td colspan=3 class=out title='" + WebCalendar.regInfo + "' align='right'><input name=btnEnter style=' background-color: " +
         WebCalendar.btnBgColor + ";cursor: hand; padding-top: 4px;  height: 100%; border: 0' onfocus='this.blur()'" +
         " type=button value='確定' onclick='parent.getSelDate();parent.hiddenCalendar()'/><input name=btnCancel style=' background-color: " +
         WebCalendar.btnBgColor + ";cursor: hand; padding-top: 4px;  height: 100%; border: 0' onfocus='this.blur()'" +
         " type=button value='取消' onclick='parent.hiddenCalendar()'/></td></tr></table>" +
    "</td></tr><tr><td height=20  align='center' bgcolor='" + WebCalendar.lightColor + "'>" +
    "    <table border=0 cellpadding=1 cellspacing=0 width='98%'>" +
    "    <tr>" +
    "        <td width=15 height=19 title='前一小時' onclick='parent.prevH()' style= 'cursor: hand' class=bg><<</td>" +
    "        <td width =15 height=19 title='前一分鐘' onclick='parent.prevMi()' style='cursor:  hand;color:red;' class=bg><</td>" +
    "        <td width=60 id=meizzHourHead  title='選擇時' onclick='parent.funHourSelect(parseInt(this.innerText, 10))'" +
    "        onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'" +
    "        onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'></td>" +
    "    <td width=50 id=meizzMinuteHead title='選擇分' onclick='parent.funMinuteSelect(parseInt(this.innerText, 10))'" +
    "        onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'" +
    "        onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'></td>" +
    "    <td width=50 id=meizzSecondHead title='選擇秒' onclick='parent.funSecondSelect(parseInt(this.innerText, 10))'" +
    "        onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'" +
    "        onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'></td>" +
    "        <td title='下一分鐘' onclick='parent.nextMi()' style='cursor: hand;color:red;' class=bg>></td>" +
    "        <td title='下一小時' onclick='parent.nextH()' style='cursor: hand' class=bg>>></td>" +
    "</tr></table>" +
...
function calendar() //主调函数
{
    ...
    //日期格式正则表达式yyyy-mm-dd:/^(\d{1,4})(|\/)(\d{1,2})\2(\d{1,2})
...
function funHourSelect() //小时的下拉框
{
    ...
    for (var i = 0; i <= 23; i++) e.options.add(new Option(appendZero(i) + "時", i));
...
function writeCalendar() //对日历显示的数据的处理程序
{
    ...
    WebCalendar.iframe.meizzHourHead.innerText = appendZero(parseInt(h, 10)) + " 時";
...
function returnDate() //根据日期格式等返回用户选定的日期
{
    if (WebCalendar.objExport) {
        ...
        //if((calendarMode!=null)&&(calendarMode=="WithTime")){
        WebCalendar.objExport.value = returnValue;
        WebCalendar.objExport.focus();
        //}else{
        //   WebCalendar.objExport.value = returnValue.substring(0,10);
        // }
        hiddenCalendar();
    }
}
...
function getSelID() {
    ...
    for (var i = 0; i < 39; i++)    //这个循环是根据源数组写到日历里显示
    {
        if (WebCalendar.dayObj[i].bgColor.toUpperCase() == WebCalendar.lightColor) {
            return i;
        }
    }
    ...
    for (var i = 0; i < 39; i++)    //这个循环是根据源数组写到日历里显示
    {
        if (WebCalendar.dayObj[i].bgColor.toUpperCase() == WebCalendar.todayColor) {
            return i;
        }
    }
    return 1;
}
function getSelDate() //根据日期格式等返回用户选定的日期
{
    if (WebCalendar.objExport) {
        var index = getSelID();
        var returnValue;
        var a = (arguments.length == 0) ? WebCalendar.day[index].split("/") : arguments[0].split("/");
        var d = WebCalendar.format.match(/^(\w{4})(-|\/)(\w{1,2})\2(\w{1,2})$/);
        if (d == null) { alert("設定的日期格式不正確！\r\n\r\n請重新定義 WebCalendar.format ！"); return false; }
        var flag = d[3].length == 2 || d[4].length == 2; //判断返回的日期格式是否要补零
        returnValue = flag ? a[2] + d[2] + appendZero(a[1]) + d[2] + appendZero(a[0]) : a[2] + d[2] + a[1] + d[2] + a[0];
