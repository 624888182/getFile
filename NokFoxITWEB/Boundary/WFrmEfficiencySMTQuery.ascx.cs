namespace SFCQuery.Boundary
{
	...
	/// </summary>
	public partial class WFrmEfficiencySMTQuery : System.Web.UI.UserControl
	{
		...
		private string QueryByModel(string Model, string DateStart, string DateEnd) 
		{
			...
			switch(ddlRegion.SelectedIndex)
			{
				case 0:
                    ...
                    if (Model == "RCX" || Model == "DVR" || Model == "GNG" || Model == "SLG" || Model == "TWN" || Model == "DVL")
                        strTestData = "UNION (SELECT DISTINCT LINE_ID, STATION_ID, STATE_ID, COUNT(*) QTY FROM (SELECT DISTINCT PRODUCT_ID,UPPER(LINE_CODE) LINE_ID,"
                            ...
                            + " STATUS STATE_ID FROM " + Model + ".PRODUCT_HISTORY_V T WHERE STATION_CODE IN ('PW','DL','FL','BD') ";
                    else
                        if (Model == "HAIER")
                            strTestData = "UNION (SELECT DISTINCT LINE_ID, STATION_ID, STATE_ID, COUNT(*) QTY FROM (SELECT DISTINCT PRODUCT_ID,UPPER(LINE_CODE) LINE_ID,"
                                + "  DECODE(STATION_CODE,'DL','T_BT','CA','T_ET','BT','T_DT','BTWL','T_MT','PT','T_JT','B1','T_CT','B2','T_NT',STATION_CODE) STATION_ID,"
                                + " STATUS STATE_ID HAIER.PRODUCT_HISTORY_V T WHERE STATION_CODE IN ('SN','BD') ";
                        else
					        strTestData = "UNION (SELECT DISTINCT LINE_ID, STATION_ID, STATE_ID, COUNT(*) QTY FROM (SELECT DISTINCT PRODUCT_ID,UPPER(LINE_CODE) LINE_ID,"
                                +" STATION_CODE STATION_ID,"
                                +" STATUS STATE_ID FROM "+Model+".PRODUCT_HISTORY_V T WHERE  1=1 ";
					break;
				case 1:
					strRegion = "'A'";
					strHistoryTable = "MES_ASSY_HISTORY";
                    if (Model == "RCX" || Model == "DVR" || Model == "GNG" || Model == "SLG" || Model == "TWN" || Model == "DVL")
					    strTestData = "UNION (SELECT DISTINCT LINE_ID, STATION_ID, STATE_ID, COUNT(*) QTY FROM (SELECT DISTINCT PRODUCT_ID,UPPER(LINE_CODE) LINE_ID,"
						    +"STATION_CODE STATION_ID,"
                            + " STATUS STATE_ID FROM " + Model + ".PRODUCT_HISTORY_V T WHERE   STATION_CODE IN ('FL','BD','PH','PRT','CI','BL','HT','CT','CM','FC','AD','RA','QA','WT','WLA','GP') ";
                    else
                        strTestData = "UNION (SELECT DISTINCT LINE_ID, STATION_ID, STATE_ID, COUNT(*) QTY FROM (SELECT DISTINCT PRODUCT_ID,UPPER(LINE_CODE) LINE_ID,"
                            + " DECODE(STATION_CODE,'WL','A_DT','D2','T_LT','A1','A_KT','A2','A_CT','A3','A_HT','A4','A_IT','A5','A_JT') STATION_ID,"
				...
				case 2:
                    ...
                    if (Model == "RCX" || Model == "DVR" || Model == "GNG" || Model == "SLG" || Model == "TWN" || Model == "DVL")
					    strTestData = "UNION (SELECT DISTINCT LINE_ID, STATION_ID, STATE_ID, COUNT(*) QTY FROM (SELECT DISTINCT PRODUCT_ID,UPPER(LINE_CODE) LINE_ID,"
						    +" DECODE(STATION_CODE,'E2','P_BT') STATION_ID,"
                            + " STATUS STATE_ID FROM " + Model + ".PRODUCT_HISTORY_V T WHERE   STATION_CODE IN ('CF','PK') ";
                    else
                        strTestData = "UNION (SELECT DISTINCT LINE_ID, STATION_ID, STATE_ID, COUNT(*) QTY FROM (SELECT DISTINCT PRODUCT_ID,UPPER(LINE_CODE) LINE_ID,"
                            + " STATION_CODE  STATION_ID,"
            ...
            //rawdata = " SELECT 'LINE' ||TO_CHAR(ASCII(SUBSTR(A.STATION_ID, 2, 1)) - 64) LINE_ID,SUBSTR(A.STATION_ID,1,1)||'_'||SUBSTR(A.STATION_ID,3,2) STATION_ID,A.STATE_ID FROM Sfc.MES_PCBA_PANEL_HISTORY  A , Sfc.MES_PCBA_PANEL_LINK  C WHERE  TO_DATE(TO_CHAR(A.CREATION_DATE,'YYYY/MM/DD HH24:MI:SS'),'YYYY/MM/DD HH24:MI:SS')"
            //    + " BETWEEN TO_DATE(" + ClsCommon.GetSqlString(DateStart) + ",'yyyy/mm/dd hh24:mi') AND TO_DATE(" + ClsCommon.GetSqlString(DateEnd) + ",'yyyy/mm/dd hh24:mi') AND SUBSTR(A.PANEL_ID,2,20)= C.PANEL_ID AND SUBSTR(C.PRODUCT_ID,1,3) ='" + Model + "' AND SUBSTR(A.STATION_ID,2,1)<>'_') GROUP BY LINE_ID, STATION_ID, STATE_ID";

            // delete to_date to_char  by zhangjijing at 2008/10/23
            //rawdata = " SELECT 'LINE' ||TO_CHAR(ASCII(SUBSTR(A.STATION_ID, 2, 1)) - 64) LINE_ID,SUBSTR(A.STATION_ID,1,1)||'_'||SUBSTR(A.STATION_ID,3,2) STATION_ID,A.STATE_ID FROM Sfc.MES_PCBA_PANEL_HISTORY  A , Sfc.MES_PCBA_PANEL_LINK  C WHERE  TO_DATE(TO_CHAR(A.CREATION_DATE,'YYYY/MM/DD HH24:MI:SS'),'YYYY/MM/DD HH24:MI:SS')"
            //    + " BETWEEN TO_DATE(" + ClsCommon.GetSqlString(DateStart) + ",'yyyy/mm/dd hh24:mi') AND TO_DATE(" + ClsCommon.GetSqlString(DateEnd) + ",'yyyy/mm/dd hh24:mi') AND SUBSTR(A.PANEL_ID,2,20)= C.PANEL_ID";

            rawdata = " SELECT 'LINE' ||TO_CHAR(ASCII(SUBSTR(A.STATION_ID, 2, 1)) - 64) LINE_ID,SUBSTR(A.STATION_ID,1,1)||'_'||SUBSTR(A.STATION_ID,3,2) STATION_ID,A.STATE_ID FROM Sfc.MES_PCBA_PANEL_HISTORY  A , Sfc.MES_PCBA_PANEL_LINK  C WHERE  A.CREATION_DATE "
                + " BETWEEN TO_DATE(" + ClsCommon.GetSqlString(DateStart) + ",'yyyy/mm/dd hh24:mi') AND TO_DATE(" + ClsCommon.GetSqlString(DateEnd) + ",'yyyy/mm/dd hh24:mi') AND SUBSTR(A.PANEL_ID,2,20)= C.PANEL_ID";
           ...
           //rcx dvr gng ?
			...
			if (Model.CompareTo("")!=0)
			{
                if (Model == "RCX" || Model == "DVR" || Model == "GNG" || Model == "SLG" || Model == "TWN" || Model == "DVL")
                {
                    ...
                    if (Model == "HAIER")
                    {
                        rawdata = rawdata + " AND PRODUCT_ID IN (SELECT  S.PRODUCT_ID FROM Sfc.MES_PCBA_PANEL_DETAIL R,Sfc.MES_PCBA_PANEL_LINK S,SHP.CMCS_SFC_SORDER T WHERE S.PANEL_ID=R.PANEL_ID AND R.WO_NO=T.SORDER AND T.MODEL ='HER')";
                        strTestData = strTestData + " AND PRODUCT_ID IN (SELECT  S.PRODUCT_ID FROM Sfc.MES_PCBA_PANEL_DETAIL R,Sfc.MES_PCBA_PANEL_LINK S,SHP.CMCS_SFC_SORDER T WHERE S.PANEL_ID=R.PANEL_ID AND R.WO_NO=T.SORDER AND T.MODEL = 'HER')";
                    }
                    else
                    {
		...
		private String GetStationList(string Model, string Region) 
		{
            ...
            station_list = "SELECT DISTINCT STATE_ID, DECODE(STATE_ID,'P',STATION_ID,STATION_ID_O1) STATION_ID,MODEL_ALIGN  "
				+ " FROM MES_COMM_ROUTE "
            ...
            sequence_list = "SELECT DISTINCT SEQUENCE_ID, STATION_ID,'P'STATE_ID,MODEL_ALIGN  "
			...
			sqlstr = "SELECT B.SEQUENCE_ID, A.STATION_ID, A.STATE_ID "
				+ "FROM (" + station_list + ") A "
				+ ",(" + sequence_list + ") B "
                + "WHERE A.STATION_ID=B.STATION_ID  AND A.MODEL_ALIGN=B.MODEL_ALIGN AND A.MODEL_ALIGN='20'  "
			...
			foreach(DataRow rw in dt.Rows)
            {
                ...
                if (rw["STATION_ID"].ToString().Substring(0, 1) != "S" && rw["STATION_ID"].ToString() != "S_AI" && rw["STATION_ID"].ToString() != "T_FO")
                {
                    SelectStr = SelectStr + ","
                        + "SUM("
                        + "DECODE(STATION_ID"
...
//		}
		...
		private void AddTableCellClicking(HtmlTableRow htr,HtmlTableRow htr1,int intColumns,string strColumnName,string strStatus)
		{
            ...
            if ((!(strColumnName.Substring(3, 1).Equals("T")) && !(strColumnName.Substring(2, 2).Equals("FO")) && (strColumnName.Substring(0, strColumnName.Length - 2).Equals("S_AI"))) || strColumnName.Substring(0, strColumnName.Length - 2).Equals("A_BI")) 
			{
			...
			else
			{
                switch (strColumnName.Substring(0, strColumnName.Length - 2).ToUpper())
				{
					...
					default:
                        AddStationCell(htr, htr1, strColumnName.Substring(0, strColumnName.Length - 2).ToUpper(), strStatus);
						break;
				} 
			}
		}	
		...
		private void FullTableData(DataTable dt)
		{
            ...
            string strModel=ddlType.SelectedValue.ToString();
				...
				for(int i=0;i<dt.Columns.Count;i++)
				{
                    ...
                    if (i != 0)
                    {
                        if (rw["LINE_ID"].ToString() == "")
                            ...
                            htc.InnerHtml = "<a href=\"/SFCQuery/Boundary/WFrmPIDDetail.aspx?" + strTemp + "&StationID=" + dt.Columns[i].ColumnName.Substring(0, dt.Columns[i].ColumnName.Length - 2) + "&LineID=" + rw["LINE_ID"].ToString() + "&Status=" + dt.Columns[i].ColumnName.Substring(dt.Columns[i].ColumnName.Length - 1, 1) + "\" target=\"_blank\">" + rw[i].ToString() + "</a>";
                    }
                    else
